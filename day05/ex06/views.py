...
def populate(request):
    ...
    try:
        ...
        for row in data:
            try:
                cur.execute('''INSERT INTO ex06_movies (
...
def display(request):
    ...
    try:
        ...
        cur.execute('''SELECT 
            ...
            producer, release_date, created, updated
            FROM ex06_movies
            ORDER BY episode_nb
            ''')
        data = cur.fetchall()
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid() and request.POST['select']:
                print(request.POST['opening_crawl'], request.POST['select'])
                cur.execute("UPDATE ex06_movies SET opening_crawl = %s WHERE episode_nb = %s;", (request.POST['opening_crawl']
                    , request.POST['select']))
                conn.commit()
        cur.execute('''SELECT 
            episode_nb, title
        ...
        return HttpResponse('1No data available', e)
    except Exception as e:
        return HttpResponse('No data available', e)
    finally:
        if conn and not conn.closed:
            conn.close()
    if data:
        return render(request, 'ex06/update.html', {'data': data, 'form': form})
    else:
        return HttpResponse('No data available')
